...
from src.data.datasets.mae_dataset import MAEDataset, build_mae_dataset
...
class KoLeoLoss(torch.nn.Module):
    ...
    def forward(self, student_output, eps=1e-8):
        ...
        with torch.amp.autocast('cuda', enabled=True):
            student_output = torch.nn.functional.normalize(student_output, eps=eps, p=2, dim=-1)
            I = self.pairwise_NNs_inner(student_output)  # noqa: E741
            distances = self.pdist(student_output, student_output[I])  # BxD, BxD -> B
            loss = -torch.log(distances + eps).mean()
        return loss
    
class KoLeoLossMatrixNorm(torch.nn.Module):
    """
    Kozachenko-Leonenko entropic loss regularizer for matrix norm 
    """
    def __init__(self):
        super().__init__() 
        self.pdist = torch.nn.PairwiseDistance(2, eps=1e-8)
    ...
    def forward(self, student_output, eps=1e-8):
        """
        Args:
            student_output (BxHWxD): backbone output of student
        """
        with torch.amp.autocast('cuda', enabled=True):
            B,H,D = student_output.shape
            norm = student_output.norm(p='fro', dim=(1,2), keepdim=True)
            student_output = student_output / norm
            flattened_output = student_output.view(B, -1)
            dots = torch.mm(flattened_output, flattened_output.t())
            dots.fill_diagonal_(-1)
            _, I = torch.max(dots, dim=1)
            distances = (student_output - student_output[I]).norm(p=2, dim=(1,2))
            loss = -torch.log(distances + eps).mean()
        return loss

class CustomTrainer(Trainer):
    def __init__(self, conf, use_koleo, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.conf = conf
        self.use_koleo = use_koleo
        if self.use_koleo:
            self.koleo_loss = KoLeoLossMatrixNorm()
            self.koleo_coeff = kwargs.get('koleo_coeff', 0.1)
            if is_rank0():
                logger.info(f'Using KoLeo loss with coefficient {self.koleo_coeff}')
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        ...
        if self.use_koleo:
            ps = model.module.ps # Since we are using DDP, we need to access the module attribute
            all_embeddings = self.gather_tensor(ps)
            koleo_loss = self.koleo_loss(all_embeddings)
            loss_total += self.koleo_coeff * koleo_loss
            
            # log every logging_steps
            if self.state.global_step > 0 and self.state.global_step % self.args.logging_steps == 0:
                if is_rank0():  
                    logger.info(f'KoLeo loss @ {self.args.logging_steps}/{self.state.global_step}: {koleo_loss.item():.4f}')
                    wandb.log({'train/koleo_loss': koleo_loss.item()}, step=self.state.global_step)
    ...
    def gather_tensor(self, _tensor):
        if not torch.distributed.is_initialized():
            return _tensor
        tensors_gather = [torch.zeros_like(_tensor) for _ in range(torch.distributed.get_world_size())]
        torch.distributed.all_gather(tensors_gather, _tensor)
...
def train_mae(conf):
    ...
    USE_KOLEO = True if not hasattr(conf.training, 'use_koleo') else conf.training.use_koleo
    kwargs = {'unimodal_training': UNIMODAL_TRAINING, 'use_koleo': USE_KOLEO}
    if hasattr(conf, 'kwargs') and conf.kwargs is not None:
        kwargs.update(conf.kwargs)
        if is_rank0():
