...
class MaskRatioCallBack(TrainerCallback):
    def on_epoch_begin(self, args, state, control, **kwargs):
        ...
        if is_rank0():
            logger.info(f"New mask ratio: {mask_ratio_dict['mask_ratio']}")
...
def train_mae(conf):
    ...
    if is_rank0():
        ...
        logger.info(f'Loaded model from {conf.experiment.dir}/{conf.training.post_training_model_name}/checkpoints/{checkpoint}')
    mae_model.cuda()
...
if __name__ == "__main__":
    ...
    if hasattr(conf, 'training') and hasattr(conf.training, 'post_training_model_name'):
        conf.training.post_training_model_name = cli_conf.training.post_training_model_name
        if is_rank0():
            logger.info(f'Post-training model name: {conf.training.post_training_model_name}')
    else:
        if is_rank0():
            logger.warning('No post-training model name specified, checking for experiment name')
        if hasattr(conf, 'experiment') and hasattr(conf.experiment, 'name'):
            conf.training.post_training_model_name = conf.experiment.name
            if is_rank0():
                logger.info(f'Switching to Post-training model name: {conf.experiment.name}')
        else:
            raise ValueError('No post-training model name specified, please specify as a command line argument')


    if is_rank0():
        logger.info(OmegaConf.to_yaml(conf))

    conf.experiment.name = f'PostMasking_{conf.training.post_training_model_name}'
    ...
    if hasattr(conf, 'masking'):
        ...
        mask_ratio_update_strategy, mask_ratio_dict = get_mask_ratio(conf.masking.update_strategy, list(conf.masking.initial_ratio), list(conf.masking.final_ratio),
                                                                    conf.training.epochs, epoch_threshold=ec)                                       
