...
if __name__ == "__main__":
    ...
    conf.experiment.name = f"PostTrain_{conf.training.post_training_model_name}"
    if is_rank0():
        logger.info(f'Experiment name: {conf.experiment.name}')


    conf.image_info.image_section_size = [256, 256]
    if is_rank0():
        logger.info(f'Using image section size {conf.image_info.image_section_size} for post-training')
