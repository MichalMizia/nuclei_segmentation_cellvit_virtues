...
import importlib
print("\n" + "="*60)
print("TRAINING - SP + Boundary Attention")
print("\n" + "="*60)
...
decoder, train_losses, val_losses, val_dices = train_loop(
    ...
    num_classes=10,
    device=device,
    save_path=os.path.join(checkpoint_dir, "_best_boundary_att.pth"),
...
print("\n" + "="*60)
print("FINAL RESULTS SUMMARY - Boundary Attention")
print(f"Max Validation Dice: {max_dice:.4f}")
print(f"Avg Validation Dice: {avg_dice:.4f}")
print(f"Metrics saved to: {os.path.join(checkpoint_dir, '_training_boundary_att.npz')}")
print("="*60)

np.savez(
    os.path.join(checkpoint_dir, "_training_boundary_att.npz"),
