...
def detect_global_context_kwarg():
    candidates = [
        "use_global_context",
        "use_global_context_block",
        "global_context_block",
        "global_context",
        "use_global_context_attention",
    ]
    sig = inspect.signature(CellViTDecoder.__init__)
...
def run_one_experiment(
    ...
    boundary_attention, use_masked_attention, use_global_context,
    num_epochs, checkpoint_dir, class_weights=None
):
    print("\n" + "=" * 90)
    print(f"[{fold_name}] {name}")
    print(
        f"  original_channels={original_channels} | boundary={boundary_attention} | "
        f"masked={use_masked_attention} | global_context={use_global_context} | epochs={num_epochs}"
    )
    print("=" * 90)
    ...
    gc_kw = detect_global_context_kwarg()
    if use_global_context:
        if gc_kw is None:
            raise ValueError(
                "You enabled global_context, but CellViTDecoder.__init__ has no known global-context kwarg. "
                "Search CellViTDecoder.__init__ signature and add the correct kwarg name to detect_global_context_kwarg()."
            )
        decoder_kwargs[gc_kw] = True
    else:
        if gc_kw is not None:
            decoder_kwargs[gc_kw] = False

    decoder = CellViTDecoder(**decoder_kwargs).to("cuda")
    ...
    # IMPORTANT: call train_loop exactly like your good file (NO val_loader kwarg)
    ...
    return {
        ...
        "global_context": use_global_context,
        "original_channels": original_channels,
        "best_dice": best_dice,
...
def main():
    ...
    experiments = [
        ("SP + boundary + masked", True,  True,  False),
        ("SP + boundary + global_context", True, False, True)
    ]
    ...
    for fold_name in ["fold1", "fold2"]:
        ...
        for name, boundary_attention, masked_attention, global_context in experiments:
    ...
    out_path = os.path.join(checkpoint_dir, "fileB_results_masked_vs_global_context.pkl")
    with open(out_path, "wb") as f:
        pkl.dump(results, f)

    print("\n" + "=" * 90)
    print("FILE B SUMMARY")
    for r in results:
        print(f"{r['fold']:<6} | {r['name']:<30} | best={r['best_dice']:.4f} @ {r['best_epoch']}")
    print(f"\nSaved -> {out_path}")
    print("=" * 90)
